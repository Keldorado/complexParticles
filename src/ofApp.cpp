...
//--------------------------------------------------------------
void ofApp::colorChanged(float &forceColor){
    myColor.setHue(colorSlide); //when the slider is updated, update the value of force in our instance of Params
}
...
void ofApp::draw(){
    ...
    ofFill();
    for (int i=0; i<p.size(); ++i) {
        p[i].draw(myColor); //draw all the particles
    }
