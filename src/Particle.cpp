...
void Particle::draw(ofColor _myColor){
    if(live){
        float size = ofMap(fabs(time-lifeTime/2), 0, lifeTime/2, 3, 1); //make it smaller the older it gets
        
        float hue = ofMap(time, 0, lifeTime, 0, 255); //shift the hue according to the age of the particle
        _myColor.setHue(hue+_myColor.getHue()); // anything after . is a function of what ever came before the .
        ofSetColor(_myColor);
